...
// Интерфейс Store объединяет базовые SQL-запросы (через встроенный интерфейс Querier)
// и операции по переводу средств в рамках транзакций.
type Store interface {
	// Интерфейс Querier предоставляет доступ к сгенерированным методам для работы с базой данных.
	Querier
	// TransferTx выполняет операцию перевода средств между двумя аккаунтами в рамках транзакции.
	// Параметры:
	//   ctx - контекст запроса, содержащий информацию о дедлайнах, отменах и т.д.
	//   arg - структура TransferTxParams с параметрами перевода (идентификаторы аккаунтов и сумма перевода).
	// Возвращает:
	//   TransferTxResult - результат выполнения перевода, включающий детали операции,
	//   error - ошибку, если операция не была выполнена успешно.
